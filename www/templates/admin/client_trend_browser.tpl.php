...
echo url('admin/client_trend.get_browser');
echo '">' . "\r\n" . '    <h3 class="heading">客户端属性 <span class="h3span"> <a href="';
echo url('admin/client_trend.get_browser?timerange=' . $get_timerange['day']);
echo '">今天</a> | <a href="';
echo url('admin/client_trend.get_browser?timerange=' . $get_timerange['yesterday']);
echo '">昨天</a> | <a href="';
echo url('admin/client_trend.get_browser?timerange=' . $get_timerange['7day']);
echo '">最近7天</a> | <a href="';
echo url('admin/client_trend.get_browser?timerange=' . $get_timerange['30day']);
echo '">最近30天</a> | <a href="';
echo url('admin/client_trend.get_browser');
...
echo url('admin/client_trend.get_browser?timerange=' . $get_timerange['day'] . '');
echo '" class="tab-btn  tab-state-active">浏览器</a>' . "\r\n\t" . '  <a href="';
...
echo '" class="tab-btn isp">网络提供商</a>' . "\r\n\t" . '  <a href="';
...
for ($i = 0; $i < count($browser_sum); $i++) {
	if ($i == 0) {
		echo '                {' . "\r\n" . '                    name: \'';
		echo ucfirst($browser_sum[$i]['browser']);
		echo '\',' . "\r\n" . '                    y: ';
		echo $browser_sum[$i]['num'];
		echo ',' . "\r\n" . '                    sliced: true,' . "\r\n" . '                    selected: true' . "\r\n" . '                }' . "\r\n\t\t\t\t";

		if ($i < (count($browser_sum) - 1)) {
			echo ', ';
		}
	}
	else {
...
for ($i = 0; $i < count($browser_kernel_sum); $i++) {
	if ($i == 0) {
		echo '                {' . "\r\n" . '                    name: \'';
		echo $browser_kernel_sum[$i]['kernel'];
		echo '\',' . "\r\n" . '                    y: ';
		echo $browser_kernel_sum[$i]['num'];
		echo ',' . "\r\n" . '                    sliced: true,' . "\r\n" . '                    selected: true' . "\r\n" . '                }' . "\r\n\t\t\t\t";

		if ($i < (count($browser_kernel_sum) - 1)) {
			echo ', ';
		}
	}
	else {
