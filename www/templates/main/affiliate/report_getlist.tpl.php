...
<div id="left">
  <div class="subnav">
    ...
    <ul class="subnav-menu">
      <li <?php if($timerange==$get_timerange['day']) echo "class='current'"?>> <a href="<?php echo url("affiliate/report.get_list?type=all&timerange=".$get_timerange['day'])?>">综合报告(当天)</a> </li>
      <li <?php if($timerange==$get_timerange['7day']) echo "class='current'"?>> <a href="<?php echo url("affiliate/report.get_list?type=all&timerange=".$get_timerange['7day'])?>">综合报告(一周)</a> </li>
      <li <?php  if($timerange==$get_timerange['30day']) echo "class='current'"?>> <a href="<?php echo url("affiliate/report.get_list?type=all&timerange=".$get_timerange['30day'])?>">综合报告(一月)</a> </li>
      <?php if(in_array('cps',$stats_type)){?>
       <li <?php //if($de_plantype==$t1) echo "class='current'"?>> <a href="<?php echo url("affiliate/orders.get_list?timerange=".$get_timerange['day'])?>">CPS明细报表</a> </li>
      <?php }?>
      <?php if(in_array('cpa',$stats_type)){?>
       <li <?php //if($de_plantype==$t1) echo "class='current'"?>> <a href="<?php echo url("affiliate/cpa_report.get_list?timerange=".$get_timerange['day'])?>"> CPA明细报表</a> </li>
       <?php }?>
    </ul>
  </div>
</div>
  ...
  <div class="box" >
    <div class="box-title" style="height:76px;">
      ...
      <div class="button" style="float:left">
        ...
        <label>
          ...
          项目报告</label>
        <label>
          <input name="report_view_type" type="radio" value="site" <?php if($type=='site')echo " checked"?>/>
          网站报告</label>
        <label>
          <input name="report_view_type" type="radio" value="zone" <?php if($type=='zone')echo " checked"?>/>
          广告位报告</label>
        </span> </div>
    ...
    <div class="box-content mt30"  style="margin-left:20px">
      <select name="" onchange="location.href = this.options[selectedIndex].value">
        <option value="<?php echo url("affiliate/report.get_list?&type=".$type."&timerange=".$timerange)?>">全部数据</option>
        <?php 
			foreach((array)$view_type_data as $p) { 
			?>
        <option value="<?php echo url("affiliate/report.get_list?&type=".$type."&id=".$p[$to_id_type]."&timerange=".$timerange)?>" <?php if($id==$p[$to_id_type])echo " selected"?> ><?php echo $p[$to_name]?> #<?php echo $p[$to_id_type]?> </option>
        <?php } ?>
      </select>
      或搜索ID：
