...
<div id="left">
  <div class="subnav">
    ...
    <ul class="subnav-menu">
      ...
      <li class='current'> <a href="<?php echo url("affiliate/orders.get_list?timerange=".$get_timerange['day'])?>">CPS明细报表</a> </li>
      <?php if(in_array('cpa',explode(',', $GLOBALS['C_ZYIIS']['stats_type']))){?>
      <li> <a href="<?php echo url("affiliate/cpa_report.get_list?timerange=".$get_timerange['day'])?>"> CPA明细报表</a> </li>
      <?php }?>
    </ul>
  </div>
</div>
      ...
      <table class="table">
        ...
        <tbody>
          <?php foreach((array)$orders as $o) {
		  		 $p =  dr ( 'affiliate/plan.get_one' ,$o["planid"]);
...
$(".menubutton").on('click', function(option) { 
	 ...
	 location.href = '<?php echo url("affiliate/orders.get_list?&status=".$status."&timerange=")?>' + d;
...
$(".stausbnt").on('click', function(option) { 
	   ...
	   location.href = '<?php echo url("affiliate/orders.get_list?&timerange=".$timerange."&status=")?>' + val;
});
