...
class users_mod extends app_models
{
	...
	public function get_list()
	{
		...
		$where = array('serviceid' => (int) $_SESSION['service']['uid'], 'type' => 1);
	...
	{
		$where = array('serviceid' => (int) $_SESSION['service']['uid'], 'date_format(regtime,\'%Y-%m-%d\')' => DAYS, 'type' => 1);
		$this->where($where);
	...
	{
		$where = array('serviceid' => (int) $_SESSION['service']['uid'], 'status' => 0, 'type' => 1);
		$this->where($where);
		$data = $this->find_count();
		$this->ar_where = array();
		return $data;
	...
	{
		$where = array('uid' => (int) $uid, 'serviceid' => (int) $_SESSION['service']['uid']);
		$data = array('status' => 2);
		$this->where($where);
		$this->set($data);
		$data = $this->update();
