...
<div id="left">
  <div class="subnav">
    ...
    <ul class="subnav-menu">
      ...
      <li> <a href="<?php echo url("advertiser/orders.get_list?timerange=".$get_timerange['day'])?>">CPS明细报表</a> </li>
       <?php }?>
      <li class='current'> <a href="<?php echo url("advertiser/cpa_report.get_list?timerange=".$get_timerange['day'])?>"> CPA明细报表</a> </li>
    </ul>
  </div>
</div>
      ...
      <table class="table">
        ...
        <tbody>
          <?php foreach((array)$cpa_report as $c) {
		  		 $p =  dr ( 'advertiser/plan.get_one' ,$c["planid"]);
...
$(".menubutton").on('click', function(option) { 
	 ...
	 location.href = '<?php echo url("advertiser/cpa_report.get_list?&status=".$status."&timerange=")?>' + d;
...
$(".stausbnt").on('click', function(option) { 
	   ...
	   location.href = '<?php echo url("advertiser/cpa_report.get_list?&timerange=".$timerange."&status=")?>' + val;
});
