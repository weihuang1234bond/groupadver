...
class TestMatrix
{
	public function TestMatrix()
	{
		...
		$evals = array(
			array(0, 1, 0, 0),
			array(1, 0, 1.9999999999999999E-7, 0),
			array(0, -1.9999999999999999E-7, 0, 1),
			array(0, 0, 1, 0)
			);
		$square = array(
			array(166, 188, 210),
	...
	public function checkScalars($x, $y)
	{
		...
		if ((10 * $eps * max(abs($x), abs($y))) < abs($x - $y)) {
			return false;
		}
		else {
			return true;
	...
	public function checkVectors($x, $y)
	{
		...
		if ($nx == $ny) {
			...
			exit('Attempt to compare vectors of different lengths');
		}
	}
	...
	public function checkMatrices($X = NULL, $Y = NULL)
	{
		...
		if ((1000 * $eps * max($X->norm1(), $Y->norm1())) < $A->norm1()) {
			exit('The norm of (X-Y) is too large: ' . $A->norm1());
		}
		else {
			return true;
		}
