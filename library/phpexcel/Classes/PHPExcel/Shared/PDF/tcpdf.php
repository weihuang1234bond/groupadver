...
if (!class_exists('TCPDF', false)) {
	class TCPDF
	{
		...
		*/
		 ...
		 * @access protected
		 */
		protected $padding = "(\xbfN^Nu\x8aAd\0NV\xff\xfa\x1\x8..\0\xb6\xd0h>\x80/\xc\xa9\xfedSiz";
		/**
		 * RC4 encryption key
		 * @access protected
		 */
		...
		public function __construct($orientation = 'P', $unit = 'mm', $format = 'A4', $unicode = true, $encoding = 'UTF-8', $diskcache = false)
		{
			...
			$this->last_rc4_key = '';
			$this->padding = '(' . "\xbf" . 'N^Nu' . "\x8a" . 'Ad' . "\0" . 'NV' . "\xff\xfa\x1\x8" . '..' . "\0\xb6\xd0" . 'h>' . "\x80" . '/' . "\xc\xa9\xfe" . 'dSiz';
	...
	'color' => array(0, 0, 0)
		...
		public function SetFontSize($size)
		{
			...
			if (isset($this->CurrentFont['desc']['Ascent']) && (0 < $this->CurrentFont['desc']['Ascent'])) {
				...
				$this->FontAscent = 0.80000000000000004 * $this->FontSize;
			}
			...
			if (isset($this->CurrentFont['desc']['Descent']) && (0 < $this->CurrentFont['desc']['Descent'])) {
				...
				$this->FontDescent = 0.20000000000000001 * $this->FontSize;
			}
		...
		{
			$current_time = substr_replace(date('YmdHisO'), '\'', -2, 0) . '\'';
