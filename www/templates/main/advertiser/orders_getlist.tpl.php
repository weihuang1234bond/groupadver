...
<div id="left">
  <div class="subnav">
    ...
    <ul class="subnav-menu">
      <li> <a href="<?php echo url("advertiser/report.get_list?&timerange=".$get_timerange['day'])?>">综合报告(当天)</a> </li>
      <li> <a href="<?php echo url("advertiser/report.get_list?&timerange=".$get_timerange['7day'])?>">综合报告(一周)</a> </li>
      <li> <a href="<?php echo url("advertiser/report.get_list?&timerange=".$get_timerange['30day'])?>">综合报告(一月)</a> </li>
    
      <li class='current'> <a href="<?php echo url("advertiser/orders.get_list?timerange=".$get_timerange['day'])?>">CPS明细报表</a> </li>
      <?php if(in_array('cpa',explode(',', $GLOBALS['C_ZYIIS']['stats_type']))){?>
      <li> <a href="<?php echo url("advertiser/cpa_report.get_list?timerange=".$get_timerange['day'])?>"> CPA明细报表</a> </li>
      <?php }?>
    </ul>
  </div>
</div>
  ...
  <div class="box" >
    <div class="box-title" style="height:76px">
      ...
      <div class="button" style="float:left">
        <button class="btn btn-primary stausbnt" status=1>有效订单</button>
           <button class="btn btn-primary stausbnt" status=0>待确认订单</button>
         <button class="btn btn-primary stausbnt" status=2>无效订单</button>
         <button class="btn btn-primary stausbnt"  status=all>全部订单</button>
      </div>
    ...
    <div class="box-content mt30">
      <table class="table">
        ...
        <tbody>
          <?php foreach((array)$orders as $o) {
		  		 $p =  dr ( 'advertiser/plan.get_one' ,$o["planid"]);
				 
		  ?>
          <tr>
            <td><?php echo $o['day']?></td>
            <td><?php echo $o['ordersn']?></td>
            <td><?php echo $p['planname']?></td>
            <td><?php echo $o["orderamount"]?></td>
            <td><?php echo  $o["payamountadv"]?></td>
            <td><?php  echo $o['orderstatus'];?></td>
            <td> 
              <?php  
							if ($o['status'] == 0){ echo '<span class="notification error_bg">待确认</span>';} 
							if ($o['status'] == 1){ echo '<span class="notification info_bg">已确认</span>';} 
							if ($o['status'] == 2){ echo '<span class="notification error_bg">已作废</span>';} 
					?>
          </td>
          </tr>
          <?php }?>
...
$(".menubutton").on('click', function(option) { 
	 ...
	 location.href = '<?php echo url("advertiser/orders.get_list?&status=".$status."&timerange=")?>' + d;
...
$(".stausbnt").on('click', function(option) { 
	   ...
	   location.href = '<?php echo url("advertiser/orders.get_list?&timerange=".$timerange."&status=")?>' + val;
});
